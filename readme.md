...
The project includes `cryptography` because `pypdf` needs its crypto extra for AES-encrypted PDFs. The official pypdf documentation recommends `pypdf[crypto]` for AES encryption/decryption; this project lists `cryptography` directly to keep the dependency easy to understand.
...
## PDF encryption troubleshooting

If you previously deployed the app with an older `requirements.txt`, redeploy after installing the updated dependencies:

```bash
pip install -r requirements.txt
```

The updated requirements include `cryptography`, which pypdf uses for AES-encrypted PDF decryption. Without the crypto dependency, pypdf can raise a `DependencyError` while decrypting an encrypted PDF.

The app now handles problematic PDFs gracefully:

- Normal PDFs are extracted normally.
- Encrypted PDFs are tested with an empty password first.
- Password-protected PDFs are skipped with a clear warning instead of crashing the application.
- Corrupted or unsupported PDFs are also reported as per-file warnings.
- Other uploaded documents can still be processed if one PDF fails.

If the PDF requires a password, remove the password from the PDF first and upload the unlocked copy.
